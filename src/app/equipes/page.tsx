...
export default function EquipesPage() {
  ...
  const removeTeam = async (id: string) => {
    // Limpa membros ao excluir equipe
    setTeamMembers((prev) => {
      const updated = { ...prev };
      delete updated[id];
      saveTeamMembers(updated);
                          ...
                          {members.length === 0 ? (
                            ...
                            <div className="mb-2 flex flex-wrap gap-1.5">
                              {members.map((name) => (
                                <span
                                  key={name}
                                  className="flex items-center gap-1 rounded-full border border-card-border bg-card-bg px-2 py-0.5 text-xs text-foreground"
                                >
                                  {name}
                                  <button
